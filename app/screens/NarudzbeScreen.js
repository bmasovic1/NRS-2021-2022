...
const NarudzbeScreen = ({ navigation }) => {
  ...
  const { state, listaProizvoda, ListaNarudzbi, listaArtikalaUNaru } = useContext(AuthContext);

  useFocusEffect(
    React.useCallback(() => {
      ListaNarudzbi()
      //listaProizvoda()  
      setLista(state.list11);
  ...
  const ItemRender = ({ narudzba }) => (
    <ListItem bottomDivider containerStyle={{ margin: 3 }}>
      ...
      <Icon
        ...
        type="font-awesome"
        onPress={() => listaArtikalaUNaru(narudzba.naziv)}
      />
    </ListItem>
  );
      ...
      <Provider>
        <SafeAreaView style={styles.container}>
          ...
          <FAB
            ...
            icon="plus"
            onPress={() =>
              navigation.navigate("NarudzbeDodaj")
            }
          />
        </SafeAreaView>
      </Provider>
